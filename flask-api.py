...
def getEmp():
    if request.args.get('employee_code')==None:
        return jsonify({'message': 'employee code not passed.'}), 400
    flag=0
    for emp in empDB:
...
def addExpense():
    ...
    if isinstance(a, (int, float)):
        pass
    elif a.isdigit() or (a.replace('.','',1).isdigit() and a.count('.') < 2):
        pass
    else:
    ...
    elif request.json.get('expense_amount').isdigit():
        pass
    else:
        return jsonify({'message': 'expense amount not an integer.'}), 400
    ...
    'expense_amount': int(request.json['expense_amount'])
    }
